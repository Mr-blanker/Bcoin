...
const actions = {
    ...
    //更新性别
    [types.UPDATE_SEX]({dispatch}, sex) {
        return new Promise((resolve) => {
            http.get('/api/user/edit.sex', {params: sex}).then(res => {
                dispatch(types.USER_USERINFO)
                return resolve(res.data)
            })
        })
    ...
    //更新手机
    [types.UPDATE_PHONE]({dispatch}, phone) {
        return new Promise((resolve) => {
            http.get('/api/user/edit.phone', {params: phone}).then(res => {
                dispatch(types.USER_USERINFO)
                return resolve(res.data)
            })
        })
